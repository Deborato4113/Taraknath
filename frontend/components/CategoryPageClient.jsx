  ...
  return (
    <div className="flex flex-col divide-y divide-gray-200 border border-gray-200">
      {items.map((item, i) => (
        <Link
          key={item.slug ?? i}
          href={`/products/${category}/${item.slug ?? i}`}
          target="_blank"
  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-10 sm:py-14 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
          <div>
          ...
          {category === "fabrication" && (
            <div className="sm:text-right flex-shrink-0">
              <p className="mono text-[10px] font-semibold tracking-[0.25em] text-blue-800 uppercase mb-1">
                Our Customer
              </p>
              <p className="text-gray-700 text-xs sm:text-sm font-semibold heading-font uppercase">
      ...
      {meta.kind === "bearing" && (
        <div className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-5 sm:px-8 pt-6 sm:pt-8">
            <p className="text-gray-500 text-sm sm:text-base">
              Journal Bearings and Thrust Pads – Complete verified by D.P and Ultrasonic Test in-house
            </p>
          </div>
        </div>
      )}
